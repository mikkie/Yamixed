	...
	<div>
		...
		<form id="saveArticleForm" class="form-horizontal" role="form" method="post" action="${rc.getContextPath()}/fav/updateArticle/${channel.id}/${article.tag.id}/${article.id}">
			...
			<div class="form-group">
				...
				<div class="col-sm-8" name="tag">
					<div id="tagSelect" class="btn-group" style="float:left;">
						...
						<input type="hidden" id="oldTag" value="${article.tag.name}"/>
			...
			<div class="form-group">
				...
				<div class="col-sm-8">
                    <#if article.privated>    
                    <input type="checkbox" name="privated" checked="checked"/> 
                    <#else>
						...
						<div class="media-body">
							<div class="media-body-left" style="width:80%;">
								<a href="${link.url}" target="_blank" class="mixLink">
									<h4 class="media-heading">
										...
										<span class="info"> 
											${link.createTime}
										<#if link.privated?? && link.privated>    
                                         未公开 
                                        </#if>
										</span>
									</h4>
							...
							<div class="media-body-right">
								<span class="input-group-btn">
									<button type="button" data="${link.id}" class="delLink btn btn-danger"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></button>
								</span>
							</div>
						</div>
					</div>
